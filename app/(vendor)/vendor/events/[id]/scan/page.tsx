...
export default function VendorQRScannerPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader back title="QR Scanner" description="Scan tickets at the door" />
      ...
      {scanState === "success" && result && (
        <Card className="bg-green-500/10 border border-green-500/30 text-center">
          <CardContent className="space-y-4">
            ...
            <Button onClick={reset} className="mt-4">Scan Next</Button>
          </CardContent>
        </Card>
      )}
      ...
      {scanState === "used" && result && (
        <Card className="bg-red-500/10 text-center">
          <CardContent className="space-y-4">
            ...
            <p className="text-lg">{result.tier}</p>
            {result.usedAt && (
              <p className="text-xs uppercase">Used at {formatDate(result.usedAt)}</p>
            )}
            <Button onClick={reset} className="mt-4">Scan Next</Button>
          </CardContent>
        </Card>
      ...
      {(scanState === "idle" || scanState === "scanning") && (
        <>
            ...
            <CardContent>
              ...
              {!cameraActive && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-6">
                  ...
                  <p className="text-sm uppercase text-muted-foreground">Camera inactive</p>
                </div>
              )}
              {cameraActive && (
          ...
          {cameraError && <p className="text-xs text-orange-400 text-center">{cameraError}</p>}
          ...
          <div className="flex gap-3">
            ...
            <Button onClick={() => setShowManual((v) => !v)} variant="outline" size="icon-lg">
          ...
          {showManual && (
            <Card>
              <CardContent className="space-y-4">
                ...
                />
                <Button onClick={() => handleScan(manualCode)} disabled={!manualCode || processing} className="w-full">
